...
| **에이전트 CLI** | `claude` ([Claude Code](https://docs.claude.com/en/docs/claude-code/overview))와 `codex` ([OpenAI Codex](https://github.com/openai/codex))가 npm으로 사전 설치. 자격증명은 박히지 않음 — 최초 사용 시 `claude` 또는 `codex` 실행으로 로그인. 빌드 시 `CLAUDE_CODE_VERSION` / `CODEX_VERSION` ARG로 버전 고정 가능. |
