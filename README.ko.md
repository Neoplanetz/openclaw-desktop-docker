...
|-----------|---------|
| **기본 OS** | Ubuntu 24.04 |
| **지원 아키텍처** | `linux/amd64`, `linux/arm64` (multi-arch manifest — Docker가 호스트 아키에 맞춰 자동 선택) |
| **데스크톱** | XFCE4 (한국어 + CJK + 이모지 폰트 포함) |
| **원격 접속** | TigerVNC + NoVNC (웹), xRDP (원격 데스크톱), VNC |
| **브라우저** | amd64: Google Chrome / arm64: Chromium (Google이 `chrome-stable` arm64를 배포하지 않아 Chromium으로 대체. OpenClaw의 CDP 자동화는 둘 다 동일하게 동작). 양쪽 모두 `--no-sandbox` 래퍼 포함. |
| **런타임** | Node.js 22 (NodeSource) |
| **OpenClaw** | npm 최신 버전, 기본 설정 사전 구성, Gateway 자동 시작, 스킬 설치를 위한 사용자 로컬 npm prefix |
| **에이전트 CLI** | `claude` ([Claude Code](https://docs.claude.com/en/docs/claude-code/overview))가 npm으로 사전 설치. 자격증명은 박히지 않음 — 최초 사용 시 `claude` 실행으로 로그인. 빌드 시 `CLAUDE_CODE_VERSION` ARG로 버전 고정 가능. (OpenAI Codex CLI는 이전 버전에 포함되었으나 OpenClaw 코어/플러그인 API 불일치로 v1.4.8에서 제외됨. 필요시 `npm install -g @openai/codex`로 직접 설치.) |
...
| 아이콘 | 기능 |
|------|-------------|
| **OpenClaw Setup** | `openclaw onboard` 실행 — AI 모델/인증, 채널(Telegram, Discord 등), 스킬을 구성합니다. 마지막의 Gateway 데몬 설치는 systemctl 심을 통해 깔끔하게 완료됩니다. |
...
| 문제 | 해결 방법 |
|-------|----------|
| systemd 없음 | `systemctl` 심이 systemd-user 호출을 직접 프로세스 관리로 변환; 엔트리포인트는 VNC, xRDP 기동을 담당 |
