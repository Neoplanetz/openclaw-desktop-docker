...
| **代理 CLI** | `claude`（[Claude Code](https://docs.claude.com/en/docs/claude-code/overview)）通过 npm 预装。镜像不包含凭证，首次使用时运行 `claude` 登录即可。构建时可用 ARG `CLAUDE_CODE_VERSION` 固定版本。（OpenAI Codex CLI 之前也曾随镜像捆绑，因 OpenClaw 核心/插件 API 不匹配在 v1.4.8 中移除。如需使用请自行 `npm install -g @openai/codex`。） |
