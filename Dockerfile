...
# packages across container recreates. Without this, a stale openclaw in
# ~/.npm-global would shadow the image-baked version on upgrade.
RUN mkdir -p /var/openclaw-npm \
    && echo "prefix=/var/openclaw-npm" > /home/${USER}/.npmrc \
    && chown -R ${USER}:${USER} /var/openclaw-npm \
